...
#define OLED8 (0x78 >> 1)    // 7 bit slave-adress without r/w-bit   Se escribe como variable para usar 2 pantallas
#define OLEDA (0x7A >> 1)																									//Cambio reciente
...
#define DISPLAY_HEIGHT        64
    ...
    void lcd_command(uint8_t cmd[], uint8_t size, uint8_t ADDRESS);    // transmit command to display
    void lcd_data(uint8_t data[], uint16_t size, uint8_t ADDRESS);    // transmit data to display
    void lcd_init(uint8_t dispAttr, uint8_t DIRECCION);
	void lcd_home(uint8_t DIRECCION);                            // set cursor to 0,0
    void lcd_invert(uint8_t invert, uint8_t DIRECCION);        // invert display
    void lcd_sleep(uint8_t sleep,  uint8_t DIRECCION);            // display goto sleep (power off)
    void lcd_set_contrast(uint8_t contrast, uint8_t DIRECCION);    // set contrast for display
    void lcd_puts(const char* s, uint8_t DIRECCION);            // print string, \n-terminated, from ram on screen (TEXTMODE)
    // or buffer (GRAPHICMODE)
    void lcd_puts_p(const char* progmem_s, uint8_t DIRECCION);        // print string from flash on screen (TEXTMODE)
    // or buffer (GRAPHICMODE)
    
    void lcd_clrscr(uint8_t DIRECCION);                // clear screen (and buffer at GRFAICMODE)
    void lcd_gotoxy(uint8_t x, uint8_t y, uint8_t DIRECCION);        // set curser at pos x, y. x means character,
    // y means line (page, refer lcd manual)
    void lcd_goto_xpix_y(uint8_t x, uint8_t y, uint8_t DIRECCION); // set curser at pos x, y. x means pixel,
    // y means line (page, refer lcd manual)
    void lcd_putc(char c, uint8_t DIRECCION);                // print character on screen at TEXTMODE
	...
	void lcd_display(uint8_t DIRECCION);                // copy buffer to display RAM
    ...
    void lcd_display_block(uint8_t x, uint8_t line, uint8_t width, uint8_t DIRECCION); // display (part of) a display line
#endif
