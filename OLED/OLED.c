...
void lcd_command(uint8_t cmd[], uint8_t size, uint8_t ADDRESS) {
#if defined I2C
    i2c_start((ADDRESS << 1) | 0);
...
void lcd_data(uint8_t data[], uint16_t size, uint8_t ADDRESS) {
#if defined I2C
    i2c_start((ADDRESS << 1) | 0);
...
void lcd_init(uint8_t dispAttr, uint8_t DIRECCION){
#if defined I2C
    i2c_init();
#elif defined SPI
    ...
    lcd_command(commandSequence, sizeof(commandSequence), DIRECCION);
    lcd_clrscr(DIRECCION);
}



void lcd_gotoxy(uint8_t x, uint8_t y, uint8_t DIRECCION){
    x = x * sizeof(FONT[0]);
    lcd_goto_xpix_y(x,y, DIRECCION);
}


void lcd_goto_xpix_y(uint8_t x, uint8_t y, uint8_t DIRECCION){
...
void lcd_clrscr(uint8_t DIRECCION){
#ifdef GRAPHICMODE
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        memset(displayBuffer[i], 0x00, sizeof(displayBuffer[i]));
        lcd_gotoxy(0,i, DIRECCION);
        lcd_data(displayBuffer[i], sizeof(displayBuffer[i]), DIRECCION);
    }
#elif defined TEXTMODE
    ...
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        lcd_gotoxy(0,i, DIRECCION);
        lcd_data(displayBuffer, sizeof(displayBuffer), DIRECCION);
    }
#endif
    lcd_home(DIRECCION);
}
...
void lcd_home(uint8_t DIRECCION){
    lcd_gotoxy(0, 0, DIRECCION);
}




void lcd_invert(uint8_t invert, uint8_t DIRECCION){
    ...
    lcd_command(commandSequence, 1, DIRECCION);
}



void lcd_sleep(uint8_t sleep, uint8_t DIRECCION){
    uint8_t commandSequence[1];
    if (sleep != YES) {
...
void lcd_set_contrast(uint8_t contrast, uint8_t DIRECCION){
...
void lcd_putc(char c, uint8_t DIRECCION){
    switch (c) {
        case '\b':
            // backspace
            lcd_gotoxy(cursorPosition.x-charMode, cursorPosition.y, DIRECCION);
            lcd_putc(' ', DIRECCION);
            lcd_gotoxy(cursorPosition.x-charMode, cursorPosition.y, DIRECCION);
            break;
        case '\t':
            ...
            if( (cursorPosition.x+charMode*4) < (DISPLAY_WIDTH/ sizeof(FONT[0])-charMode*4) ){
                lcd_gotoxy(cursorPosition.x+charMode*4, cursorPosition.y, DIRECCION);
            }else{
                lcd_gotoxy(DISPLAY_WIDTH/ sizeof(FONT[0]), cursorPosition.y, DIRECCION);
            }
            break;
        case '\n':
            // linefeed
            if(cursorPosition.y < (DISPLAY_HEIGHT/8-1)){
                lcd_gotoxy(cursorPosition.x, cursorPosition.y+charMode, DIRECCION);
            }
            break;
        case '\r':
            // carrige return
            lcd_gotoxy(0, cursorPosition.y, DIRECCION);
            break;
        default:
            ...
            if (charMode == DOUBLESIZE) {
                ...
                lcd_data(data, sizeof(FONT[0])*2, DIRECCION);
                ...
                lcd_data(data, sizeof(FONT[0]), DIRECCION);
                cursorPosition.x += sizeof(FONT[0]);
            }
#endif
            break;
...
void lcd_puts(const char* s, uint8_t DIRECCION){
    while (*s) {
        lcd_putc(*s++, DIRECCION);
    }
}


void lcd_puts_p(const char* progmem_s, uint8_t DIRECCION){
    register uint8_t c;
    while ((c = pgm_read_byte(progmem_s++))) {
        lcd_putc(c, DIRECCION);
    }
}
...
void lcd_display(uint8_t DIRECCION) {
#if defined (SSD1306) || defined (SSD1309)
    lcd_gotoxy(0,0, DIRECCION);
    lcd_data(&displayBuffer[0][0], DISPLAY_WIDTH*DISPLAY_HEIGHT/8, DIRECCION);
#elif defined SH1106
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        lcd_gotoxy(0,i, DIRECCION);
        lcd_data(displayBuffer[i], sizeof(displayBuffer[i]), DIRECCION);
...
void lcd_display_block(uint8_t x, uint8_t line, uint8_t width, uint8_t DIRECCION) {
    if (line > (DISPLAY_HEIGHT/8-1) || x > DISPLAY_WIDTH - 1){return;}
    if (x + width > DISPLAY_WIDTH) { // no -1 here, x alone is width 1
        width = DISPLAY_WIDTH - x;
    }
    lcd_goto_xpix_y(x,line, DIRECCION);
    lcd_data(&displayBuffer[line][x], width, DIRECCION);
}
#endif
